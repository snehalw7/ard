...
<legend class="text-center">No. of Ph.D Research Scholars at BITS-Pilani, Hyderabad campus as on <?php echo date("d.m.Y"); ?></legend>
	...
	<tbody>
		...
		?>
			...
			<?php 
				$full1 = "SELECT count(*) from phd_scholar, phd_fellowship where phd_scholar."."type = 'full time' AND phd_scholar.dept = '".$dept['dept']."' AND phd_fellowship.type = 'institute fellowship';";

				$full2 = "SELECT count(*) from phd_scholar, phd_fellowship where phd_scholar."."dept = '".$dept['dept']."' AND phd_scholar.type = 'full time' AND phd_fellowship.type = 'project fellows extension support from institute'";

				$full3 = "SELECT count(*) from phd_scholar, phd_fellowship where phd_scholar."."dept = '".$dept['dept']."' AND phd_scholar.type = 'full time' AND phd_fellowship.type = 'no fellowship'";

				$full4 = "SELECT count(*) from phd_scholar, phd_fellowship where phd_scholar."."dept = '".$dept['dept']."' AND phd_scholar.type = 'full time' AND phd_fellowship.type = 'own fellowship'";

				$full5 = "SELECT count(*) from phd_scholar, phd_fellowship where phd_scholar."."dept = '".$dept['dept']."' AND phd_scholar.type = 'full time' AND phd_fellowship.type = 'project fellow'";

				$full6 = "SELECT count(*) from phd_scholar, phd_fellowship where phd_scholar."."dept = '".$dept['dept']."' AND phd_scholar.type = 'full time' AND phd_fellowship.type = 'others'";
				...
				$part = "SELECT count(*) from phd_scholar where dept = '".$dept['dept']."' AND type = 'part time';";

				$count_part = mysqli_query($conn, $part);
				$count_part = mysqli_fetch_assoc($count_part)['count(*)'];

				$aspirant = "SELECT count(*) from phd_scholar where dept = '".$dept['dept']."' AND type = 'aspirant';";

				$count_aspirant = mysqli_query($conn, $aspirant);
				$count_aspirant = mysqli_fetch_assoc($count_aspirant)['count(*)'];

				$lecturer = "SELECT count(*) from phd_scholar where dept ='".$dept['dept']."' AND type = 'lecturer';";
